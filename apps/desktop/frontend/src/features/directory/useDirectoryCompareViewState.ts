...
export function useDirectoryCompareViewState({
  ...
  const directoryTreeCacheRef = useRef<Record<string, DirectoryCompareItem[]>>({})
  ...
  const loadDirectoryChildren = async (relativePath: string): Promise<DirectoryCompareItem[]> => {
