...
function buildEntry(compareModeHint: DirectoryCompareItem['compareModeHint']): DirectoryCompareItem {
  return {
    name: 'file.json',
...
describe('useDirectoryCompareChildDiffActions', () => {
  ...
  it('returnToDirectoryCompare restores context and switches mode', async () => {
    ...
    act(() => {
      result.current.returnToDirectoryCompare()
    })
